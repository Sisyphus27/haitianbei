...
class Runner:
    def __init__(self, env, args):
        ...
        self.rolloutWorker = RolloutWorker(
            env, self.agents, args)   # [FIX] 创建 rolloutWorker
    ...
    def run(self, alg):
        ...
        for epoch in range(self.args.n_epoch):
            ...
            for episode_idx in range(self.args.n_episodes):
                episode, train_reward, train_time, _, _, train_move_time = self.rolloutWorker.generate_episode(
                    episode_idx)
    ...
    def export_schedule_csv(self, episodes_situation, filename="schedule.csv"):
        ...
        with open(fpath, "w", newline="", encoding="utf-8") as f:
            ...
            for t, jid, sid, pid, pmin, mmin in sorted(episodes_situation, key=lambda x: x[0]):
                ...
                w.writerow([f"{t:.2f}", code, jid, sid, pid,
                           f"{pmin:.2f}", f"{mmin:.2f}"])
    ...
    def evaluate(self):
        ...
        for epoch in range(self.args.evaluate_epoch):
            _, episode_reward, episode_time, win_tag, for_gant, episode_move_time = self.rolloutWorker.generate_episode(
                epoch, evaluate=True)
        ...
        if self.args.load_model and not self.args.learn:
            file_name = f"evaluate.json"      # [FIX] 修正拼写
            file_path = os.path.join(self.save_path, file_name)
            with open(file_path, 'w') as f:
                json.dump(self.results, f, indent=4)
