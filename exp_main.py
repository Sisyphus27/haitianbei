...
class Exp_main(Exp_Basic):
    def __init__(self, args):
        ...
        if not os.path.isdir(self.decomp_base_model_dir):
            _logging.warning(f"[DECOMP] 模型目录不存在，回退到 base_model_dir: {self.decomp_base_model_dir} -> {self.base_model_dir}")
        ...
        try:
            ...
            if self.reset_kg and self.kg_service is not None:
                try:
                    ...
                    _logging.info("[KG] (init) reset_graph 已执行，已清理历史动态关系，仅保留固定节点。")
                except Exception:
                    pass
            ...
            if self.kg_service:
                try:
                    _snap = self.kg_service.graph_snapshot() or {}
                    _logging.info(f"[KG] (init) snapshot nodes={_snap.get('nodes_count', 0)} edges={_snap.get('edges_count', 0)}")
                except Exception:
                    pass
    ...
    def _build_model(self):
        ...
        def _load_handle(model_dir: str, lora_dir: Optional[str], tag: str) -> Dict[str, Any]:
            ...
            # 检测模型类型（通过目录名或 config.json）- 仅支持文本模型
            is_glm4 = False
            try:
                model_name = os.path.basename(model_dir)
                if "GLM-4" in model_name or "glm4" in model_name.lower():
                    is_glm4 = True
                ...
                if os.path.isfile(cfg_path):
                    ...
                    with open(cfg_path, "r", encoding="utf-8") as _f:
                        cfg = _json.load(_f)
                    model_type_str = str(cfg.get("model_type", "")).lower()
                    if "glm" in model_type_str or "chatglm" in model_type_str:
                        is_glm4 = True
            except Exception:
                pass
            
            # 检查模型目录和 tokenizer 文件
            if not os.path.isdir(model_dir):
                raise RuntimeError(f"[MODEL] {tag} 模型目录不存在: {model_dir}")
            
            tokenizer_files = ["tokenizer.json", "tokenizer_config.json", "vocab.txt", "vocab.json"]
            has_tokenizer_file = any(os.path.isfile(os.path.join(model_dir, f)) for f in tokenizer_files)
            if not has_tokenizer_file:
                _logging.warning(f"[MODEL] {tag} 模型目录中未找到常见的 tokenizer 文件，但继续尝试加载")
            
            # 加载 tokenizer
            try:
                _logging.info(f"[MODEL] {tag} 开始加载 tokenizer from {model_dir}")
                tok = AutoTokenizer.from_pretrained(
                    model_dir, trust_remote_code=True, use_fast=False
                )
                _logging.info(f"[MODEL] {tag} tokenizer 加载完成，类型: {type(tok)}")
            except Exception as e:
                _logging.error(f"[MODEL] {tag} tokenizer 加载异常: {e}")
                raise RuntimeError(f"[MODEL] {tag} tokenizer 加载失败: {e}") from e
            
            # 立即验证 tokenizer 是正确的对象
            if tok is None:
                raise RuntimeError(f"[MODEL] {tag} tokenizer 为 None")
            if isinstance(tok, bool):
                # 如果返回布尔值，尝试使用其他方式加载（可能是 GLM-4 的特殊情况）
                _logging.warning(f"[MODEL] {tag} tokenizer 返回了布尔值 {tok}，尝试使用备用方法加载")
                try:
                    # 尝试使用 fast tokenizer
                    tok = AutoTokenizer.from_pretrained(
                        model_dir, trust_remote_code=True, use_fast=True
                    )
                    _logging.info(f"[MODEL] {tag} 使用 fast tokenizer 加载成功")
                except Exception as e2:
                    _logging.error(f"[MODEL] {tag} fast tokenizer 也加载失败: {e2}")
                    # 尝试直接指定 tokenizer 类（GLM-4 可能需要）
                    try:
                        if is_glm4:
                            # GLM-4 可能需要使用特定的 tokenizer 类
                            from transformers import PreTrainedTokenizer  # type: ignore
                            # 尝试从 config 获取 tokenizer_class
                            ...
                            if os.path.isfile(cfg_path):
                                ...
                                if tok_class_name:
                                    _logging.info(f"[MODEL] {tag} 尝试使用 tokenizer_class: {tok_class_name}")
                                    # 这里我们仍然使用 AutoTokenizer，但可能需要其他参数
                                    tok = AutoTokenizer.from_pretrained(
                                        model_dir, trust_remote_code=True, use_fast=False,
                                        local_files_only=False
                                    )
                        else:
                            raise RuntimeError(f"[MODEL] {tag} tokenizer 返回了布尔值，且备用方法也失败")
                    except Exception as e3:
                        raise RuntimeError(f"[MODEL] {tag} tokenizer 加载失败，返回了布尔值 {tok}。所有备用方法都失败: {e3}") from e3
                
                # 再次验证
                if isinstance(tok, bool) or tok is None or not hasattr(tok, 'encode'):
                    raise RuntimeError(f"[MODEL] {tag} tokenizer 仍然无效，类型: {type(tok)}")
            
            if not hasattr(tok, 'encode'):
                raise RuntimeError(f"[MODEL] {tag} tokenizer 加载失败，类型错误: {type(tok)}，缺少 encode 方法")
            if not callable(tok):
                raise RuntimeError(f"[MODEL] {tag} tokenizer 不可调用，类型错误: {type(tok)}")
            
            # 记录 tokenizer 类型信息（用于调试）
            try:
                _logging.info(f"[MODEL] {tag} tokenizer 类型: {type(tok).__name__}")
            except Exception:
                pass
            
            # 安全地设置 pad_token（兼容 GLM-4 和原有模型）
            try:
                # 检查 pad_token 是否存在且有效
                if not hasattr(tok, 'pad_token') or tok.pad_token is None:
                    if hasattr(tok, 'eos_token') and tok.eos_token is not None:
                        ...
                        # GLM-4 特殊处理：尝试添加 pad_token
                        try:
                            tok.add_special_tokens({'pad_token': '[PAD]'})
                        except Exception:
                            pass
                # 额外检查：如果 pad_token 是布尔值或其他非预期类型，重置它
                elif isinstance(tok.pad_token, bool):
                    # pad_token 不应该是布尔值，重置它
                    if hasattr(tok, 'eos_token') and tok.eos_token is not None:
                        tok.pad_token = tok.eos_token
                    elif hasattr(tok, 'unk_token') and tok.unk_token is not None:
                        tok.pad_token = tok.unk_token
            except Exception as e:
                _logging.warning(f"[MODEL] tokenizer pad_token 设置异常: {e}")
                # 最后尝试：直接设置 eos_token 作为 pad_token
                try:
                    if hasattr(tok, 'eos_token') and tok.eos_token is not None:
                        tok.pad_token = tok.eos_token
                except Exception:
            ...
            # 文本模型：使用 AutoModelForCausalLM（支持 GLM-4-9B-0414 和 Qwen 等）
            ...
            if tok is None:
                raise RuntimeError(f"[MODEL] {tag} tokenizer 为 None")
            if not hasattr(tok, 'encode'):
                raise RuntimeError(f"[MODEL] {tag} tokenizer 类型错误: {type(tok)}")
    ...
    def run(self):
        ...
        if mode == "marl-train":
            ...
            if not os.path.isfile(event_jsonl):
                _logging.warning(f"[MARL] 指定的事件JSONL文件不存在: {event_jsonl}")
        ...
        if not events_file or not os.path.isfile(events_file):
            ...
            if os.path.isfile(_default_ev):
                ...
                _logging.error("[STREAM] 缺失 --events_file")
        ...
        if not bool(getattr(self.args, "no_warmup", False)):
            try:
                self.warmup_models(
                    judge=True,
                    decomp=not bool(getattr(self.args, "no_warmup_decomp", False)),
                    order=str(getattr(self.args, "warmup_order", "judge_first")),
                    timeout_sec=int(getattr(self.args, "warmup_timeout_sec", 300)),
                )
            except Exception:
                pass
        ...
        self._judge_out_file = os.path.join(self.judge_out_dir, f"stream_{self._session_ts}.jsonl")
        self._decomp_out_file = os.path.join(self.decomp_out_dir, f"stream_{self._session_ts}.jsonl")
    ...
    *,
    ...
    conflict_judge: bool = True,
        ...
        if not conflict_judge:
            # === 任务一：仅做时空/历史/态势解析，输出标准 JSON（精简版，提升速度） ===
            instruction = (
                "你是一个严格的时空信息解析器。\n"
            ...
            # 限制KG文本长度（减少prompt长度）
            kg_limited = kg_text.strip() if kg_text else ""
            if kg_limited and len(kg_limited) > 1000:
                lines = kg_limited.split("\n")
                kg_limited = "\n".join(lines[:50])  # 最多50行KG信息
    ...
    def train_rules_lora(
        ...
        try:
            ...
            _logging.warning(f"[TRAIN] tokenizer pad_token 设置异常: {e}")
            try:
                if hasattr(tok, 'eos_token') and tok.eos_token is not None:
                    tok.pad_token = tok.eos_token
            except Exception:
    ...
    def _generate_with_handle(
        ...
        *,
        ...
        # 验证并获取 tokenizer
        tok = handle.get("tokenizer")
        if tok is None:
            raise RuntimeError(f"模型 {model_key} 的 tokenizer 未初始化")
        # 确保 tok 是 tokenizer 对象，不是布尔值或其他类型
        if not hasattr(tok, 'encode') or not hasattr(tok, '__call__'):
            raise RuntimeError(f"模型 {model_key} 的 tokenizer 类型错误: {type(tok)}")
        
        # 验证并获取 model
        model = handle.get("model")
        if model is None:
            raise RuntimeError(f"模型 {model_key} 的 model 未初始化")
        ...
        # 安全地获取 pad_token_id 和 eos_token_id
        pad_token_id = None
        try:
            if hasattr(tok, 'pad_token_id') and tok.pad_token_id is not None:
                pad_token_id = tok.pad_token_id
            elif hasattr(tok, 'pad_token') and tok.pad_token is not None and not isinstance(tok.pad_token, bool):
                pad_token_id = tok.convert_tokens_to_ids(tok.pad_token)
        except Exception:
            pass
        ...
        if len(prompts) == 1:
            ...
            if duration > 1.0:
                _logging.info(f"[GEN] {model_key} 生成耗时 {duration:.2f}s | 输入tokens={input_token_count} 生成tokens={gen_token_count} | 速度={gen_token_count/duration:.1f} tokens/s")
    ...
    def _get_kg_context(self, focus_entities: Optional[List[str]] = None, limit: int = 200) -> str:
        ...
        参数:
        - limit: 限制返回的KG信息条数，默认200
        """
        if not self.kg_service:
            return "【KG状态】\n(离线模式，未加载图谱)"
        try:
            return self.kg_service.get_context_text(focus_entities=focus_entities, limit=limit)
        except Exception:
            return "【KG状态】\n(离线模式，服务异常)"
    
    def _save_output_and_update_kg(
        self, event: str, output: str, parsed: Optional[dict] = None, conflict_judge: bool = True
    ) -> None:
        """保存输出并更新KG（辅助方法，减少重复代码）。"""
        ...
        if conflict_judge and parsed:
            payload["compliance"] = parsed.get("compliance")
            payload["reasons_len"] = parsed.get("reasons_len")
            if not bool(parsed.get("parsed")):
                ...
                try:
                    payload["output"] = __json.loads(output) if isinstance(output, str) else output
                except Exception:
                    payload["output_raw"] = output
        else:
            try:
                payload["output"] = __json.loads(output) if isinstance(output, str) else output
            except Exception:
        ...
        if conflict_judge:
            ...
            try:
                _kg_chk = self.kg_service.check_event_conflicts(event) or {}
                kg_reasons_cnt = len(_kg_chk.get("reasons", []) or [])
            except Exception:
                pass
            
            trigger_policy = getattr(self.args, "marl_trigger_policy", "llm_and_kg")
            llm_conflict = bool(parsed and parsed.get("compliance") == "冲突")
            ...
            if is_conflict:
                try:
                    if getattr(self.kg_service, "kg", None) is not None:
                        self._marl_rectify_and_update_kg(self.kg_service.kg, result_name_suffix="auto")
                except Exception:
                    try:
        ...
        if self.enable_kg_vis:
            try:
                ...
                self.kg_vis_idx += 1
                ts = int(__t.time())
                out_png = os.path.join(self.kg_vis_dir, f"kg_{ts}_{self.kg_vis_idx:04d}.png")
    ...
    def _process_single_event(
        ...
        try:
            ...
            try:
                ...
                pass
            out = None
        gen_duration = __t.monotonic() - gen_start
        ...
        if not conflict_judge:
            ...
            if out is None or not out.strip():
                ...
                retry_tokens = min(max_gen_tokens * 2, 256)  # 进一步减少重试token数
                try:
                    retry_start = __t.monotonic()
                    outs = self._generate_with_handle(
                        "judge",
                        [prompt],
                    ...
                    gen_duration += retry_duration
                    if retry_duration > 0.1:
                        _logging.info(f"[GEN-RETRY] 重试耗时 {retry_duration:.2f}s")
                except Exception:
                    out = ""
            
            # 保存输出（不需要解析"合规/冲突"标签）
            save_start = __t.monotonic()
            try:
                import json as __json
                ...
                pass
            save_duration = __t.monotonic() - save_start

            # 更新 KG（不需要检查冲突，直接更新）
            # 优化：对于抽取模式，可以简化KG更新（跳过复杂的生命周期侧写）
            kg_start = __t.monotonic()
            if self.kg_service is not None:
                try:
                    # 对于conflict_judge=False模式，仅做基础三元组更新，跳过复杂的生命周期侧写
                    # 直接调用底层的update_with_triples，跳过_apply_lifecycle_side_effects
                    if hasattr(self.kg_service, "kg") and hasattr(self.kg_service.kg, "update_with_triples"):
                        try:
                            from models.triples_extraction import extract_triples as _extract_triples
                            triples = _extract_triples(event)
                            self.kg_service.kg.update_with_triples(triples)
                        except Exception:
                            # 回退到完整更新
                            self.kg_service.extract_and_update(event)
                    else:
                    ...
                    pass
            kg_duration = __t.monotonic() - kg_start

            # KG 可视化（仅在启用时执行，避免性能损失）
            vis_duration = 0.0
            if self.enable_kg_vis and self.kg_service is not None:
                vis_start = __t.monotonic()
                try:
                    self.kg_vis_idx += 1
                    ts = int(__t.time())
                    ...
                    pass
                vis_duration = __t.monotonic() - vis_start

            total_duration = __t.monotonic() - total_start
            _logging.info(
                f"[PERF] 总耗时 {total_duration:.2f}s | "
                f"生成 {gen_duration:.2f}s ({gen_duration/total_duration*100:.1f}%) | "
                f"KG更新 {kg_duration:.2f}s ({kg_duration/total_duration*100:.1f}%) | "
                f"保存 {save_duration:.2f}s"
            )
            if vis_duration > 0.1:
                _logging.info(f"[PERF] 可视化耗时 {vis_duration:.2f}s")
        ...
        # 保存输出并更新KG（冲突判定模式）
        self._save_output_and_update_kg(event, out, parsed, conflict_judge=True)
        return out or ""
        ...
        if is_single_sample:
            ...
            for ev in events_iter:
                ...
                out = self._process_single_event(
                    ...
                    decomp_available, conflict_judge=conflict_judge_mode,
                )
            ...
            if len(batch_events) >= max(1, int(batch_size)):
                ...
                if not conflict_judge_mode:
                    ...
                    for e, o in zip(batch_events, outs):
                        self._save_output_and_update_kg(e, o, conflict_judge=False)
                        yield (e, o)
